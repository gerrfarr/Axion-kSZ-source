...
class CosmoDB(object):
    ...
    def add(self, cosmo):
        ...
        if np.any(generate_cosmo_identifier(cosmo) == self.__db['hash_value']):
            ...
            logs_path = camb_outputs_path+"logs/camb_log_ID={}".format(len(self.__db))
        ...
        return db_entry.name, db_entry['ran_TF'], db_entry['successful_TF'], db_entry['path_root']
    ...
    def get_by_id(self, ID, get_cosmo=False):
        try:
            ...
            if not get_cosmo:
                ...
                return cosmo, db_entry.name, db_entry['ran_TF'], db_entry['successful_TF'], db_entry['path_root']
